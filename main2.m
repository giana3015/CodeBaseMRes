...
for m = 1:length(mFolders)
    if ~mFolders(m).isdir, continue; end  % skip non-folders
    ...
    for d = 1:length(days)
        ...
        for c = 1:length(placeCellIdx)
            ...
            for t = 1:length(targetTrials)
                ...
                figName = sprintf('ratemap_cell%02d_trial%s.png', cell_id, trialName);
                save(fullfile(ratemapFolder, matName), 'rm');

                fig = figure('Visible', 'off');
                imagesc(rm); axis equal tight;
                set(gca, 'YDir', 'normal');
                colormap hot; colorbar;
                title(sprintf('%s — Cell %d Trial %s', session, cell_id, trialName));
                saveas(gcf, fullfile(ratemapFolder, figName));
                close(fig);
            end
            ...
            fig = figure('Visible', 'off');
            imagesc(corrMatrix, [-1 1]); axis square;
            colormap(jet); colorbar;
            xticks(1:10); yticks(1:10);
            title(sprintf('%s — Cell %d Ratemap Correlation', session, cell_id));
            saveas(gcf, fullfile(corrFolder, sprintf('corrMatrix_cell%02d.png', cell_id)));
            close(fig);
        end
    end
end